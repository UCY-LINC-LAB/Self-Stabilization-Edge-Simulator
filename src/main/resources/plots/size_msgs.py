...
def compute_graph(df):
    ...
    # To Kbytes
    df['control_msgs_sz'] /= 1024.0
    df['data_msgs_sz'] /= 1024.0
    df['time'] /= 1000 # To seconds

    controlColor='xkcd:bright blue'
    dataColor='xkcd:light orange'
    ax1.fill_between(x=df['time'], y1=df['data_msgs'], y2=0,color=dataColor, alpha=0.85, label="Data Plane")
    ax1.plot(df['time'], df['data_msgs'],color=dataColor, marker='o', markersize=2, alpha=1, linewidth=1)

    ax1.fill_between(x=df['time'], y1=df['control_msgs_sz'], y2=0,color=controlColor, alpha=0.85, label="Control Plane")
    ax1.plot(df['time'], df['control_msgs_sz'],color=controlColor, marker='D', markersize=2, alpha=1, linewidth=1)
    ...
    ax1.set_ylabel('Network Traffic (Kb/s)')
    ax1.grid()
    # Now to MBs
    df['control_msgs_sz'] /= 1024
    df['data_msgs_sz'] /= 1024

    ax2.plot(df['time'], df['data_msgs_sz'].cumsum(),color=dataColor,alpha=1, label="Data Plane")
    ax2.plot(df['time'], df['control_msgs_sz'].cumsum(),color=controlColor, alpha=1, label="Control Plane")
    # ax2.plot(df['time'], df['sr_read_sz'].cumsum(), alpha=1, label="SharedR Read")
    # ax2.plot(df['time'], df['sr_write_sz'].cumsum(), alpha=1, label="SharedR Write")
    # ax2.plot(df['time'], df['health_checks_sz'].cumsum(), alpha=1, label="Health Checks")
    ax2.legend()
    ax2.grid()
    # ax2.set_title('CDF of Bytes Transmitted')
